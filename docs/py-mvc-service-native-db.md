...
project/
  src/
    ...
    utils/
      __init__.py        # project-specific helpers; the BR calendar comes from wwdates
    config/
...
| `src/utils/` | Project helpers | The BR calendar comes from the `wwdates` dependency (wrapped by `utils.dates`) |
