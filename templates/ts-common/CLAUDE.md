...
| `optional/otel-log-emitter.ts` | `withOtelLogExport(baseEmitter)` — OTLP log export (opt-in, blueprintx#438), the TS sibling of `python-common/optional/otel_logging.py`. The ONLY module that imports `@opentelemetry/*` (dynamically, via `import()`, so a project that declines the prompt never even attempts it). Its one relative import (`import type { LogEmitter } from './log-emitter'`) is type-only and therefore erased before emit — it avoids the extension conflict `log-emitter.ts`'s own row describes because it produces no runtime import at all, so the source can live in `optional/` and still resolve correctly once copied to sit beside `log-emitter.ts` in a project's `src/utils/` (`ts-lib`) or `src/shared/utils/` (`react-spa-webpack`). Injected by each `ts_*.sh` scaffold's own `conditional_copy_otel` on the OTel prompt — see that function for the pinned dependency versions (⚠️ exact pins, not `^` ranges — the Logs signal is still "not yet stable" upstream in JS, same status as Python's). ⚠️ Unlike Node's OTLP exporter (which reads `OTEL_EXPORTER_OTLP_*` env vars itself), the BROWSER exporter takes no env-var fallback at all — this file resolves the endpoint/headers itself and passes them explicitly, so both runtimes share one resolution path (see the module's own docstring for the measurement) |
| `optional/otel-log-emitter.test.ts` | Unit tests for the two pure guard functions (`resolveSignalOverride`, `isCleartextWithCredentials`) plus `withOtelLogExport`'s synchronous contract (no endpoint → same object back; cleartext+credentials → refused with a warning; endpoint configured → every call still reaches the base emitter, including after the dynamic `@opentelemetry/*` import — genuinely unresolvable here, since those packages are not devDependencies of `ts-common` itself — rejects, exercising the real fire-and-forget path rather than a mock of it) |
