...
Scope of this file: rules that apply specifically to `infrastructure/`.
Layer import boundaries and capability flow live in the parent
`templates/react-spa-webpack/CLAUDE.md` — do not duplicate them here.
...
Every infrastructure class should declare `implements <Port>` where
`<Port>` is an interface from `../domain/ports`. The port is the
contract; the adapter is one implementation.
...
| Do | Don't |
|---|---|
| Implement a port from `../domain/ports` | Export bare functions that bypass the port |
