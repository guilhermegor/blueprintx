...
def build() -> AppContainer:
	...
	database. For persistence, build a handler with
	``chassis.db_schema.application.build_database_handler`` and swap
	``InMemoryNoteRepository`` for a ``DatabaseHandler``-backed implementation.
	"""
