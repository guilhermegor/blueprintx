...
def python_functions(path_file: pathlib.Path) -> list:
	...
	try:
		...
		# A file this gate cannot parse is a finding of its own — reporting it as "no
		# functions" would let a broken file pass by being unreadable.
		print(f"{path_file}: could not parse ({err})")
		return []
	...
	for cls_node in ast.walk(cls_tree):
		...
		list_found.append(
			(cls_node.name, cls_node.lineno, int_span - docstring_span(cls_node))
		)
	return list_found
...
def file_problems(path_file: pathlib.Path) -> list:
	...
	if path_file.suffix == ".py":
		list_functions = python_functions(path_file)
		str_note = ", docstring excluded"
	elif path_file.suffix == ".sh":
		list_functions = shell_functions(path_file)
		str_note = ""
	else:
		return []
...
def main(list_argv: list) -> int:
	...
	print(
		f"\n{len(list_problems)} function(s) over {INT_MAX_LINES} lines. Split the function "
		f"rather than raising the ceiling: the limit is about what one reader holds in their "
		f"head at once, and it does not move because one function is special. For Python the "
		f"docstring is already excluded, so the number shown is code only."
	)
	return 1
