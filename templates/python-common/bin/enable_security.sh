...
#
# blueprintx#287: .github/workflows/secret_scan.yaml (ggshield) is a SEPARATE, opt-in decision
# from the native toggles above — this script's PATCH only reaches GitHub's own scanning, it
# cannot set a third-party repo secret for ggshield, and the opt-in does not belong here: a
# project only receives that workflow when GITGUARDIAN_API_KEY was exported at SCAFFOLD time
# (bin/lib/scaffold_git_remote.sh propagates it to the new repo's secrets then, the same
# moment GH_PAT_REVIEW_TRIGGER is propagated for coderabbit_trigger.yaml). `poe init` runs
# later, against an already-created repo, so it is the wrong phase to gate a file's shipping —
# it can only ever set a secret for a workflow that either already exists or was never
# copied. A maintainer who wants ggshield on a project scaffolded without the key runs
# `gh secret set GITGUARDIAN_API_KEY --repo <owner>/<repo>` and copies the workflow in by hand;
