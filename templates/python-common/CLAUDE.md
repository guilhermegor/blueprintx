...
| `optional/typing/` | Runtime type-checking engine (`TypeChecker`, `ABCTypeCheckerMeta`, `ProtocolTypeCheckerMeta`, `@type_checker`, `validate_type`). **Always injected** — to `src/chassis/typing/` (DDD) or `src/utils/typing/` (MVC, via `chassis.typing → utils.typing` rewrite). **Backed by `beartype`** (`validate.py` is a thin adapter, not a hand-rolled walker) — do not reimplement it. Two policies via `BeartypeConf`: violations raise `TypeError` (beartype's own exception is not a `TypeError` subclass), and `bool` is **not** accepted as `int`. Behavioural notes: mocks must be `spec=`-ed to pass; container checks are **sampled O(1)**, not exhaustive |
