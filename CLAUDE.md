...
|-------|----------|----------------------|
| Domain | `modules/<feature>/domain/` | Nothing (pure Python, no I/O) |
| Application | `modules/<feature>/application/` | Domain only |
| Infrastructure | `modules/<feature>/infrastructure/` | Domain ports + external libs |
| Core | `src/core/` | Cross-cutting shared pieces only; keep lean |

`core/infrastructure/database/base.py` defines `DatabaseHandler` (ABC) — all DB handlers extend it and implement `create / read / update / delete / backup / close`.
